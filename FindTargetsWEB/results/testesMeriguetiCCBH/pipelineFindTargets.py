...
class MyThread(threading.Thread):
    ...
    def run(self):
        ...
        FindTargets().mainMethod(self.model, self.organism, self.name, self.email)
...
class FindTargets:
    ...
    def mainMethod(self, model, organismParam, name, email):
        ...
        try:
            ...
            ####################################################
            self.reportModel(model, directory+"//dados_modelo_"+str(model)+"_.xls")
            ...
            df_11 = pd.read_excel(directory + "//" + "11-hitsEncontradosUniprot.xls")
            df_13 = pd.read_excel(directory + "//" + "13-list_inhibitors_per_target.xls")

            df_merged = pd.merge(pd.merge(df_08, df_11, on='UNIPROTID', how='inner'), df_13, on='UNIPROTID', how='inner')
    ...
    def sendMailWithReportAttached(self, name, email, zip_file_report):
        mensagem = "FINAL REPORT - FIND TARGETS WEB \n\n\n" \
                   "" \
                   "YOUR ANALYSIS WAS FINISHED WITH SUCCESS AND GENERATE THE FILES:\n" \
                   "" \
                   "08-filter_ECNumbers_drugbank.xls\n" \
                   "FIELDS: EC NUMBER, PROTEIN NAME, ORGANISM NAME, UNIPROTID, DRUGBANKID\n\n" \
                   "" \
                   ...
                   "" \
                   "dados_modelo.xls\n" \
                   "ALL GENES, REACTIONS AND METABOLITES IN SBML FILE USED IN ANALYSIS\n\n" \
                   "" \
                   "resultado_final.xls\n" \
                   "FILE WITH ALL DATA FOUND IN 08, 11 AND 13 XLS FILES.\n\n" \
